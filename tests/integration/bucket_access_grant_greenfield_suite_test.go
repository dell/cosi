...
var _ = Describe("Bucket Access Grant for Greenfield Bucket", Ordered, Label("grant", "greenfield", "objectscale"), func() {
	// Resources for scenarios
	const (
		namespace string = "access-grant-namespace-greenfield"
	)
	...
	It("Creates BucketAccess with KEY authorization mechanism", func(ctx context.Context) {
		...
		awsPrincipalString := objscl.BuildPrincipalString(grantBucketAccess.Status.AccountID, Namespace)

		grantBucketPolicy := GetBucketPolicy(resourceARNs, []string{awsPrincipalString})
