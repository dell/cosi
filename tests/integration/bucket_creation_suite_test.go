...
var (
	clientset    *kubernetes.Clientset // name confusion here, both are clientsets
...
var _ = Describe("COSI driver", func() {
	...
	// Background
	BeforeEach(func() {
		By("A Kubernetes cluster is up and running")
		steps.CheckClusterAvailability(clientset)
		By("The ObjectScale platform is installed on the cluster")
		steps.CheckObjectScaleInstallation(clientset)
		By("An ObjectStore 'object-store-1' is created")
		steps.CreateObjectStore(objectscale, "object-store-1")
		By("A Kubernetes namespace 'driver-ns' is created")
		steps.CreateNamespace(clientset, "driver-ns")
		By("A Kubernetes namespace 'namespace-1' is created")
		steps.CreateNamespace(clientset, "namespace-1")
		By("COSI controller is installed in namespace 'driver-ns'")
		steps.CheckCOSIControllerInstallation(clientset, "driver-ns")
		By("COSI driver 'cosi-driver' is installed in namespace 'driver-ns'")
		steps.CheckCOSIDriverInstallation(clientset, "cosi-driver", "driver-ns")
		By(" And BucketClass resource is created from specification 'my-bucket-class'")
	...
	It("Successfully creates bucket", func() {
		...
		By("checking the status 'bucketReady' of Bucket resource referencing BucketClaim resource 'bucket-claim-valid'  is 'true' and 'bucketID' is not empty")
		steps.CheckBucketStatusAndBucketID(bucketClient, validBucket)
	})
