...
# bundle a COSI driver helm chart, installation scripts, and
...
usage() {
   echo
   echo "$0"
   echo "Make a package for offline installation of a COSI driver"
   echo
   echo "Arguments:"
   ...
   echo "               For example: my.registry.com:5000/dell/cosi"
...
if [ "${MODE}" == "helm" ]; then
  INSTALLERDIR="${REPODIR}/dell-cosi-helm-installer"
  ...
  DRIVERNAME=${DRIVERNAME:-"dell-cosi-driver"}
