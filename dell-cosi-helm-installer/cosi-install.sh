...
function usage() {
  ...
  decho "Options:"
  decho "  Required"
  decho "  --namespace[=]<namespace>                Kubernetes namespace containing the COSI driver"
  ...
  decho "  --version                                Use this version for COSI Driver Image"
  decho "  --skip-verify                            Skip the kubernetes configuration verification to use the COSI driver, default will run verification"
  decho "  -h                                       Help"
...
function header() {
  log section "Installing COSI Driver: ${DRIVER} on ${kMajorVersion}.${kMinorVersion}"
}

#
# check_for_driver will see if the driver is already installed within the namespace provided
function check_for_driver() {
  log section "Checking to see if COSI Driver is already installed"
  ...
  if [ "${1}" == "install" -a "${NUM}" != "0" ]; then
    ...
    log error "The COSI Driver is already installed"
  fi
  if [ "${1}" == "upgrade" -a "${NUM}" == "0" ]; then
    log error "The COSI Driver is not installed"
  fi
}
...
function install_driver() {
  ...
  HELMOUTPUT="/tmp/cosi-install.$$.out"
  run_command helm ${1} \
  ...
  if [ $? -ne 0 ]; then
    cat "${HELMOUTPUT}"
    log error "Helm operation failed, output can be found in ${HELMOUTPUT}. The failure should be examined, before proceeding. Additionally, running cosi-uninstall.sh may be needed to clean up partial deployments."
  fi
  log step_success
...
function verify_kubernetes() {
  ...
  if [ $VERIFY -eq 0 ]; then
    ...
    "${VERIFYSCRIPT}" --version "${VERSION}" --driver-version "${DRIVER_VERSION}" --namespace "${NS}" --release "${RELEASE}" --values "${VALUES}" ${EXTRA_OPTS}
    VERIFYRC=$?
    case $VERIFYRC in
    0) ;;
