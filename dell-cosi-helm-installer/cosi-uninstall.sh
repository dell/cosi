...
function usage() {
    ...
    decho "Options:"
    decho "  Required"
    decho "  --namespace[=]<namespace>  Kubernetes namespace to uninstall the COSI driver from"
...
function check_for_driver() {
    ...
    if [ "${NUM}" == "0" ]; then
        log uninstall_error "The COSI Driver is not installed."
        exit 1
    fi
}
...
if [ $? -ne 0 ]; then
    decho "Removal of the COSI Driver was unsuccessful"
    exit 1
fi

decho "Removal of the COSI Driver is in progress."
